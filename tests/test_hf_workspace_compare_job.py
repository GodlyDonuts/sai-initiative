...
def test_smol_workspace_comparator_is_cpu_only_and_preserves_4b_hold() -> None:
    job = (ROOT / "jobs" / "sai-compare-smollm3-3b-workspace-cpu.sbatch").read_text()
    assert "#SBATCH --gres" not in job
    assert "#SBATCH --no-requeue" in job
    assert 'case "${CUDA_VISIBLE_DEVICES:-}"' in job
    assert "sai.evaluation.hf_smol_workspace_compare" in job
    for argument in (
        "--parent-mmlu-pro",
        "--parent-musr",
        "--recurrent-mmlu-pro",
