...
def test_normalized_exact_prefers_stronger_semantic_stratum(tmp_path):
    parent = {"source_path": "data/p.parquet", "sha256": "a" * 64}
    ...
    data = shard / "candidate_descriptors.parquet"
    rows = [low, high]
    pq.write_table(pa.Table.from_pylist(rows, schema=_schema()), data)
    import hashlib
    ...
    shard_receipt = _signed(
        {
            ...
            "source": source,
            "counts": {
                "production_candidate_descriptors": 2,
                "production_candidate_text_utf8_bytes": sum(
                    row["text_utf8_bytes"] for row in rows
                ),
                "production_candidate_source_tokens": sum(
    ...
    aggregate_path = tmp_path / "aggregate.json"
    aggregate_path.write_text(json.dumps(aggregate, sort_keys=True))
    build_decision(tmp_path / "descriptors", aggregate_path, tmp_path / "exact", 1)
    connection = sqlite3.connect(tmp_path / "exact" / "normalized_exact_keep.sqlite3")
    kept = connection.execute(
        "SELECT source_row_identity_sha256 FROM keep"
    ).fetchone()[0]
    connection.close()
    assert kept == high["source_row_identity_sha256"]
