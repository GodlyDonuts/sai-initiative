...
from sai.data import common_pile_stack_edu_practical_hf_publish as publish
...
def _admission(root: Path) -> tuple[Path, dict]:
    ...
    row = {
        ...
        "source_id": "common_pile_stack_edu",
    ...
    receipt = _signed(
        {
            ...
            "counts": {"admitted_rows": 1, "admitted_text_utf8_bytes": 800},
...
def test_publish_shard_uploads_only_code_locator(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result["status"] == "complete_stack_edu_practical_hf_publish_shard"
...
def test_publish_empty_shard_does_not_upload(tmp_path: Path, monkeypatch) -> None:
    admission = tmp_path / "admission"
    _admission(admission)

    def fail_upload(*args, **kwargs):
        raise AssertionError("empty shard must not upload")
    ...
    assert result["status"] == "complete_stack_edu_practical_hf_publish_empty_shard"
