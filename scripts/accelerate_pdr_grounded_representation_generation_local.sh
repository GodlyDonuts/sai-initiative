...
# A reverse-order companion safely fills idle slots within the same local
# ceiling
...
# request contract, so completed shard summaries remain aggregate-compatible.
# Measured 12- and 8-call trials caused heavy HTTP 429s, and successful receipts
# still showed sporadic 429s at four local calls plus two Stokes calls. Three
# local calls is therefore the current throughput setting while the two-call
# Stokes Books worker shares this key.
...
run_lane() {
  ...
  local sai_summary
  local sai_attempt
  for ((sai_shard=127-sai_lane; sai_shard>=0; sai_shard-=sai_lanes)); do
    ...
    while true; do
      ...
      printf '{"event":"pdr_grounded_representation_accelerator_retry","lane":%s,"shard_index":%s,"attempt":%s}\n' \
...
if [[ "${sai_records}" != "${sai_expected_records}" \
  ...
  echo "PDR grounded-representation acceleration custody is incomplete" >&2
  exit 1
fi

printf '{"event":"pdr_grounded_representation_acceleration_complete","records":%s,"summaries":%s}\n' \
