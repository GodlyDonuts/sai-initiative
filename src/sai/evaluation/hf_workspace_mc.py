"""Evaluate one completed matched Qwen workspace arm on a Sai dev board."""
...
from sai.evaluation.hf_parent import load_text_parent, validate_mechanics_receipt
...
class HFWorkspaceEvaluationError(RuntimeError):
    """A trained workspace, parent, or benchmark binding differs."""
...
def load_completed_workspace(
    *,
    ...
    """Reopen a completed full-run receipt and exact workspace checkpoint."""
...
class HFWorkspaceChoiceAdapter(nn.Module):
    """Produce only the choice-position logits needed by normalized MC scoring."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if state_mode not in {"recurrent", "reset_average"}:
            raise HFWorkspaceEvaluationError("workspace state mode differs")
        self.system = system
        self.state_mode = state_mode

    def train(self, mode: bool = True):
        super().train(mode)
        self.system.parent.eval()
        return self

    def forward(self, input_ids: torch.Tensor, segment_ids: torch.Tensor):
        raise HFWorkspaceEvaluationError(
            "workspace evaluation requires selected choice positions"
        )
    ...
    def choice_logits(
        ...
        *,
        ...
        if (
            ...
            or token_count <= 0
            or start_position < 0
            or start_position + token_count > input_ids.shape[1]
        ):
            raise HFWorkspaceEvaluationError("workspace choice geometry differs")
        hidden = self.system.parent_hidden(input_ids, torch.ones_like(input_ids))
        selected = []
        for position in range(start_position, start_position + token_count):
            candidate, _ = self.system.logits_at(
                hidden,
                segment_ids,
                position=position,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    """Validate one full trained arm and emit a standard development result."""
    ...
    if runtime != mechanics["runtime"]:
        raise HFWorkspaceEvaluationError("live parent differs from mechanics")
    ...
    try:
        ...
        raise HFWorkspaceEvaluationError(
            "workspace development scoring failed"
        ) from error
