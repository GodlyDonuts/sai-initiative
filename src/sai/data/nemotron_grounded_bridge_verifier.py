"""Verify paired-source bridge candidates through an independent NVIDIA request."""
...
RECEIPT_SCHEMA = "sai-nemotron-grounded-bridge-verification-receipt-v1"
SUMMARY_SCHEMA = "sai-nemotron-grounded-bridge-verification-shard-summary-v1"
REASONING_EFFORT = None
OUTPUT_SUFFIX = "grounded-bridge-independent-model-family-verification"
...
def load_candidates(path: Path) -> list[dict[str, Any]]:
    if not path.is_file() or path.is_symlink() or path.stat().st_nlink != 1:
        raise NousLabelWorkerError("bridge verification population is unsafe")
    rows = []
    try:
            ...
            for line_number, line in enumerate(handle, start=1):
                try:
                    ...
                    raise NousLabelWorkerError(
                        f"bridge verification row {line_number} differs"
                    ) from error
    except (OSError, UnicodeError) as error:
        raise NousLabelWorkerError("bridge verification population differs") from error
    identities = [row["candidate_identity_sha256"] for row in rows]
    if not rows or len(identities) != len(set(identities)):
        raise NousLabelWorkerError("bridge verification identities differ")
    return rows
