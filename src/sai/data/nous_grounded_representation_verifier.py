"""Run resumable grounded-representation verification through Hermes."""
...
RECEIPT_SCHEMA = "sai-nous-grounded-representation-verification-receipt-v1"
SUMMARY_SCHEMA = "sai-nous-grounded-representation-verification-shard-summary-v1"
REASONING_EFFORT = "low"
OUTPUT_SUFFIX = "grounded-verification"
DEFAULT_CONCURRENCY = 4


def load_candidates(path: Path) -> list[dict[str, Any]]:
    """Load one exact verification population."""

    if not path.is_file() or path.is_symlink() or path.stat().st_nlink != 1:
        raise NousLabelWorkerError("verification population is unsafe")
    rows = []
    try:
            ...
            for line_number, line in enumerate(handle, start=1):
                try:
                    ...
                    raise NousLabelWorkerError(
                        f"verification population row {line_number} differs"
                    ) from error
    except (OSError, UnicodeError) as error:
        raise NousLabelWorkerError("verification population differs") from error
    identities = [row["candidate_identity_sha256"] for row in rows]
    if not rows or len(identities) != len(set(identities)):
        raise NousLabelWorkerError("verification population is empty or duplicated")
    return rows
...
def execute_one(
    ...
    *,
    ...
    """Execute one conservative source/generated verification contract."""
    ...
    return execute_contract(
        ...
        maximum_completion_tokens=2_400,
