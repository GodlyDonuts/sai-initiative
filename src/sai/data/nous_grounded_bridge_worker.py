"""Generate source-paired cross-domain connection candidates through Hermes."""
...
RECEIPT_SCHEMA = "sai-nous-grounded-bridge-receipt-v1"
SUMMARY_SCHEMA = "sai-nous-grounded-bridge-shard-summary-v1"
REASONING_EFFORT = "low"
OUTPUT_SUFFIX = "grounded-bridge"
DEFAULT_CONCURRENCY = 2


def load_candidates(path: Path) -> list[dict[str, Any]]:
    if not path.is_file() or path.is_symlink() or path.stat().st_nlink != 1:
        raise NousLabelWorkerError("grounded bridge population is unsafe")
    rows = []
    try:
            ...
            for line_number, line in enumerate(handle, start=1):
                try:
                    ...
                    raise NousLabelWorkerError(
                        f"grounded bridge row {line_number} differs"
                    ) from error
    except (OSError, UnicodeError) as error:
        raise NousLabelWorkerError("grounded bridge population differs") from error
    identities = [row["candidate_identity_sha256"] for row in rows]
    if not rows or len(identities) != len(set(identities)):
        raise NousLabelWorkerError("grounded bridge identities differ")
    return rows
